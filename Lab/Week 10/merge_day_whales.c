...
int dayMatches(struct date *dateA, struct date *dateB);


struct pod *findPod(struct pod *pod, struct pod *first_pod) {
    struct pod *curr = first_pod;
    while (curr != NULL) {
        if (strcmp(pod->species, curr->species) == 0 && dayMatches(pod->when, curr->when)) { return curr; }
        curr = curr->next;
    }
    return NULL;
}
...
void merge_day_whales(struct pod *first_pod) {
    ...
    while (curr != NULL) {
        ...
        match = findPod(curr, first_pod);

        if (match != NULL && curr != match) {
            match->how_many += curr->how_many;
            lastAddr->next = curr->next;
            free(curr->when);
            free(curr->species);
            free(curr);
            curr = lastAddr;
