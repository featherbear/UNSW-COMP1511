...
void printDateAsString(struct date *date) {
...
int dateIsInRange(struct date *date, struct date *lowerRange, struct date *upperRange);
int dateIsInRange(struct date *date, struct date *lowerRange, struct date *upperRange) {
    return dateIsBefore(lowerRange, date) && dateIsBefore(date, upperRange);
}
...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
    ...
    while (current != NULL) {
        if (dateIsInRange(current->when, start_day, finish_day)) {
            printDateAsString(current->when);
