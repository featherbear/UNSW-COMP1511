...
void species_count(char species[], struct pod *first_pod, int *n_pods, int *n_whales) {
    ...
    struct pod *currentPod = first_pod;
    while (currentPod != NULL) {
        if (strcmp(currentPod->species, species) == 0) {
            _n_pods++;
            _n_whales += currentPod->how_many;
        }
    ...
    *n_pods = _n_pods; // CHANGE ME
    *n_whales = _n_whales; // CHANGE ME
}
...
//_n_pods++;
//_n_whales += sightings[i].how_many;
//}
//i++;
//}
//
